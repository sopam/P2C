...
% Problem is that ibntervention doesnt run if we have a separate rule for direct and causal intervention. so  we try to incorporate it
% lite_direct by adding a lite causal rule
 ...
 %ICLP 
 % To avoid Repeat of Actions

   anti_member_2(_,[]).

   
   anti_member_2(X,[H|T]):- X \= H, anti_member_2(X,T). %To pass messages
  
%?-anti_member_2(2,[2,3,4]).
%?- anti_member([d,e1,f],[state(1,[a,b,c]),'<-D1',state(1,[d,e,f]),'<-D2',state(3,[g,h,i])]).
